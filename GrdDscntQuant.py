...
def grdescentquant(func, w0, stepsize, maxiter, xTr, yTr, level_w,level_q, type_w,type_q, scale,tolerance=1e-02):
    # INPUT:
    ...
    while num_iter < maxiter:
        loss, gradient = func(w, xTr, yTr, level_q,type_q, scale)
        if loss > prior_loss:
        ...
        if np.linalg.norm(gradient) < tolerance:
            break
        w = quantize(w, level_w, type_w, scale).reshape(-1, 1)
        prior_loss = loss
